import { regexLinariaSyntaxPattern } from '../constants';
 ...
 */
export const performFastCheck = (content: string, fastCheck = true): boolean => {
	// Show the fastCheck message once per build
	if (fastCheck && !fastCheckMessageShown) {
		logger.info('Linaria fastCheck optimization enabled - skipping transform for files without Linaria syntax.');
		logger.info('If you experience styling issues, try disabling fastCheck in your next.config file.');
